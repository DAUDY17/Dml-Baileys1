...
export const makeMessagesRecvSocket = (config) => {
    ...
    const handleLegacyMexNewsletterNotification = async (node) => {
        ...
        const fallbackUpdateNode = mexNode?.content ? null : getBinaryNodeChild(node, 'update') || getAllBinaryNodeChildren(node)[0];
        const payloadNode = mexNode?.content ? mexNode : fallbackUpdateNode;
                ...
                switch (tag) {
                    case 'add':
                        ...
                        updatedDevices = [
                            ...
                            ...entries.map(e => ({ user: e.user, device: e.device }))
                        ];
                        break;
                    case 'remove':
    ...
    const handleBadAck = async ({ attrs }) => {
        ...
        if (attrs.error) {
            ...
            if (attrs.error === SERVER_ERROR_CODES.MissingTcToken) {
                ...
                logger.warn({ msgId: attrs.id, from: attrs.from }, 'message rejected: sender is under an account reachout timelock restriction');
                await fetchAccountReachoutTimelock().catch(err => logger.debug({ err }, 'failed to fetch reachout timelock'));
            }
            else {
